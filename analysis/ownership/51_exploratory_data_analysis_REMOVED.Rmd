...
We see that duplicates are rarely removed - the 95th percentile is 0, and the 99th is 3.

```{r}
data |> summarize(min(REMOVED), median(REMOVED), q95(REMOVED), q99(REMOVED), max(REMOVED))
```

But there are still changes when teams remove duplicates---some teams more than others.
And some removals are large - even though most are small.

```{r}
data |> filter(REMOVED > 0) |> group_by(committerteam, REMOVED) |> tally()
```

For the most part, teams do not remove duplicates. If they do, they often remove few (single-digits).
...
The 95% quantile plots show that for many repos, and many teams in those repos, in 19 out of 20 file changes, no duplicates were removed.
The outlier is IntTest, where many teams have removed single-digit duplicates.
In the Neptune repository, we also can see some more duplicate removals by the Architect and Green teams.
...
The pattern repeats for the 99% quantile.
More duplicates are removed in IntTest, and Jupiter is now also starting to show some removals.
In the Neptune repo, however, it is still the Architect and Green teams that remove duplicates, with Blue and Brown trailing.

### Maximum removed duplicates
