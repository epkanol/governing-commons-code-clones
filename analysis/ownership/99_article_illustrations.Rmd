...
RM0_file <- "removed-M_intercepts_only.rds"
stopifnot(file.exists(cachefile(RM0_file)))

RM0 <- readRDS(cachefile(RM0_file))
...
(p <- ppc_stat_grouped(y = d$y, yrep, stat = "q99", group = d$team) + ggtitle("Posterior predictive 99% quartile per team") + theme(axis.text=element_text(size=14, angle=45))
)
```
```{r}
...
As the model has no predictors, just intercepts, it does not matter what values are used. But our onepred function will use them anyway, with the default values, so the data frame will include them. But they are not used to make inferences.

```{r plot-removals}
(p <- removals_probability(onepred(model=RM0)) + ggtitle(label=NULL, subtitle=NULL)
)
```
```{r}
figsave("99-prob-REMOVE-intercept-by-team.pdf", p)
```
